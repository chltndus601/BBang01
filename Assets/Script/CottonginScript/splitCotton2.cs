...
public class splitCotton2 : MonoBehaviour
{
    public GameObject partPrefab; // 6등분된 오브젝트의 프리팹
    ...
    void SplitObject()
    {
        ...
        {
            new Vector3(-0.1f, 0, 0.15f), // front-left
            new Vector3(0.1f, 0, 0.15f), // front-right
            new Vector3(-0.1f, 0, 0), // middle-left
            new Vector3(0.1f, 0, 0), // middle-right
            new Vector3(-0.1f, 0, -0.15f), // back-left
            new Vector3(0.1f, 0, -0.15f) // back-right
        };
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            part.transform.localScale = originalScale / 3; // 6등분이므로 스케일을 1/3로 조정
            Rigidbody partRb = part.GetComponent<Rigidbody>();
            if (partRb == null)
            {
                partRb = part.AddComponent<Rigidbody>();
            }
            partRb.mass = rb.mass / 6; // 원래 오브젝트의 질량을 6등분
